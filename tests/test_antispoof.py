...
# 1. 移除 conftest 對 src.antispoof 的 stub，改用真實模組
for key in list(sys.modules.keys()):
    if "src.antispoof" in key:
        sys.modules.pop(key, None)
# 2. stub onnxruntime，讓真實模組能在無 onnxruntime 的 CI runner 上 import
sys.modules.setdefault("onnxruntime", MagicMock())

# 3. 正常導入
from src.antispoof.antispoof import AntiSpoof  # noqa: E402
...
class TestAntiSpoof:
    ...
    def test_predict_spoof(self, mock_onnx_session_spoof):
        """測試照片/螢幕欺騙攻擊."""
        with patch("onnxruntime.InferenceSession", return_value=mock_onnx_session_spoof):
            detector = AntiSpoof(onnx_path="fake.onnx", threshold=0.6)
            dummy_img = np.zeros((128, 128, 3), dtype=np.uint8)
            result = detector.predict(dummy_img)
